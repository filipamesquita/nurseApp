...
Create GitHub issues for the `filipamesquita/nurseApp` repository using a structured
...
- **From the codebase**: run `git log --oneline -20` and scan recent files to understand
  what areas of the code are relevant. Note the tech stack (Java 17, Spring Boot, Maven,
  Onion Architecture — domain/application/infrastructure/interfaces) so the issue body
  reflects accurate technical context.
...
```bash
gh search issues --repo filipamesquita/nurseApp "<keywords>"
```
...
Use this template — it matches the style established in the Phase 0/1 user story issues
(#1–#6):
...
pulled from the codebase — which aggregate, layer, or class is involved.
...
- The **context** section does the heavy lifting — explain the current state, the desired state,
  and the gap between them. Pull specifics from the codebase (class names, package paths,
  aggregate boundaries) so the implementer has a head start
- **Acceptance criteria** should be checkboxes, each one a binary pass/fail condition.
  Aim for 3–7 items. Avoid vague criteria like "works well" — prefer "Device.isDueForReplacement()
  returns true when today is on or after the computed due date"
...
- Be specific: "Add Periodicity value object with due-date calculation" not "Device logic"
