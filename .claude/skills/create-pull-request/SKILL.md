...
Create GitHub pull requests for the `filipamesquita/nurseApp` repository
...
convention `feat/<issue-number>-<short-slug>` (e.g., `feat/1-initialize-git-repository`).
...
Examples:
- `#1 - initialize git repository and project skeleton`
- `#6 - add object-oriented design ADRs`
- `#8 - add commit-message skill`
...
- The **test plan** should be actionable steps a reviewer can follow to verify
  the change works (e.g. `mvn test`, boundary cases for value objects). Include
...
Examples:
- `feat/1-initialize-git-repository`
- `fix/12-checklist-completion-bug`
